...
def _joint_rl_dispim_uncorr(
    ...
    for _ in trange(num_iter) if verbose else range(num_iter):
        ...
        est_a = xp.multiply(est_i, conv(backproj_a, div_stable(view_a, cona, epsilon)))
        conb = conv(psf_b, est_a)
        est_i = xp.multiply(est_a, conv(backproj_b, div_stable(view_b, conb, epsilon)))
    if req_both:
...
def _additive_joint_rl_boundcorr(
    ...
    window_a = xp.where(alpha_a > boundary_sigma_a, 1.0 / alpha_a, 0.0)
    window_b = xp.where(alpha_b > boundary_sigma_b, 1.0 / alpha_b, 0.0)
    del M_a, M_b
...
def _additive_joint_rl_uncorr(
    ...
    for _ in trange(num_iter) if verbose else range(num_iter):
        ...
        est_b = xp.multiply(est_i, conv(backproj_b, div_stable(view_b, conb, epsilon)))
        est_i = (est_a + est_b) / 2
    if req_both:
        est_i[xp.logical_or(view_a == 0, view_b == 0)] = 0
    return est_i
