...
def _load_region(vol: zarr.Array, window: SliceWindow3D, xp: ModuleType = numpy):
...
def _load_and_compute_variance_queued(window: SliceWindow3D, vol: zarr.Array, queue):
    return _variance_queued(_load_region(vol, window, numpy), queue)


class ChunkwiseRegistration:
    def __init__(
        self, vol0: zarr.Array, vol1: zarr.Array, chunk_size: int | Tuple[int, int, int]
    ):
        self._vol0 = vol0
        self._vol1 = vol1
    ...
    def variance_perchunk(self, use_vol1: bool = False, verbose: bool = False):
        ...
        with concurrent.futures.ProcessPoolExecutor(
            ...
            variance = partial(
                _load_and_compute_variance_queued,
            ...
            if verbose:
                ...
                iterator = tqdm(iterator, total=n_task, desc="Chunkwise Variance")
            vars = [0 for _ in self._chunk_indices]
            for future in iterator:
                idx = future_to_idx[future]
                var = future.result()
                if verbose:
                    iterator.set_postfix_str(f"{idx:d} -> {var:.2f}")
                vars[idx] = var
            return vars
