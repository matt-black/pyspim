...
__linear_module_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "linear_cub.cu"
)
with open(__linear_module_path) as f:
    __linear_module_txt = f.read()

__cubspl_module_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "cubspl_cub.cu"
)
with open(__cubspl_module_path) as f:
    __cubspl_module_txt = f.read()

__nearest_module_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "nearest_cub.cu"
)
with open(__nearest_module_path) as f:
...
def make_kernel(
    ...
    expr1 = "rval * mval"
    if function == "nip":
        function_name = f"normalizedInnerProduct{block_size_x:d}{block_size_y:d}{block_size_z:d}"
        expr2, expr3 = "rval * rval", "mval * mval"
    elif function == "cr":
        function_name = f"correlationRatio{block_size_x:d}{block_size_y:d}{block_size_z:d}"
        expr2, expr3 = "(rval-mu_ref) * (rval-mu_ref)", "mval*mval"
    elif function == "ncc":
        function_name = f"normalizedCrossCorrelation{block_size_x:d}{block_size_y:d}{block_size_z:d}"
    ...
    return cupy.RawKernel(
        code=function_text, name=function_name, backend="nvcc", options=("-std=c++14",)
    )
