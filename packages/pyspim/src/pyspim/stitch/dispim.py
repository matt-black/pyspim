...
from ..data.dispim import uManagerAcquisitionMultiPos, StitchedDataset
...
def _load_function_2dto1d(fldr_path : os.PathLike, head : str, channel : int,
                          window : SliceWindow3D, position : int):
    dset = StitchedDataset(fldr_path, xp=cupy)
    return subtract_constant_uint16arr(
        dset.get(position, head, channel, window), 100
    )
...
class DiSPIM1DStitcher(object):
    ...
    def determine_pair_idxs(self) -> List[Tuple[int,int]]:
        if self._reduced_from_2d:
            with open(os.path.join(self._root_fldr, 'params.json'), 'r') as f:
    ...
    def register_pairs(self, 
        ...
        load_af = partial(load_fun_base, self._root_fldr, 'a', 
                          channel, slice_f)
        load_bf = partial(load_fun_base, self._root_fldr, 'b',
                          channel, slice_f)
        load_ar = partial(load_fun_base, self._root_fldr, 'a',
                          channel, slice_r)
        load_br = partial(load_fun_base, self._root_fldr, 'b',
                          channel, slice_r)
        ...
        with concurrent.futures.ProcessPoolExecutor(
            ...
            for future in iterator:
                ...
                T, res = future.result()
                if verbose:
                    string = '{:s} {:d}->{:d}, met={:.3f}'.format(
                        head, idx0, idx1, 1-res.fun
                    )
...
class DiSPIM2DStitcher(object):
    ...
    def one_dimensionalify(self,
                           out_fldr : os.PathLike, reduction_index : int,
                           reg_channel : int, 
        ...
        slice_f, slice_r = build_slices(
            self.tile_shape, overlap_axis, len_overlap
        )
        # determine launch parameters
        shp = self.tile_shape
        ...
        with concurrent.futures.ProcessPoolExecutor(
            ...
            reg_a = partial(register_pair_load_gpuq,
                            queue=gpu_queue,
                            loadfun0=load_af, loadfun1=load_ar,
                            pcc_prereg=pcc_prereg, piecewise=reg_piecewise,
                            metric=reg_metric, transform=reg_transform,
                            interp_method=interp_method, 
                            bounds=reg_bounds, kernel_launch_params=klp,
                            verbose=verbose, **opt_kwargs)
            reg_b = partial(register_pair_load_gpuq,
                            queue=gpu_queue,
                            loadfun0=load_bf, loadfun1=load_br,
            ...
            for future in iterator:
                ...
                if verbose:
                    iterator.set_postfix_str(
                        '{:s}_{:d} {:d}->{:d}, met={:.3f}'.format(
                            head, gidx, idx0, idx1, 1-res.fun
                        )
                    )
            ...
            stitch_futures = []
            for gidx, pair_idxs in enumerate(pair_idx_groups):
                out_a = [(t[2], t[3]) for t in out_list 
                        if (t[0] == 'a' and t[1] == gidx)]
                out_b = [(t[2], t[3]) for t in out_list
                        if (t[0] == 'b' and t[1] == gidx)]
                # sort the output lists
                ord = order_pair_indices(pair_idxs)
                idxoe = list(enumerate(ord))
                out_a.sort(key=lambda t: [s for s,i in idxoe if i==t[0]][0])
                ...
                klp_a = launch_params_for_volume(out_shp_a, *block_sizes)
                assemble_a = partial(assemble_stitch_and_save_gpuq,
                                     out_shp=out_shp_a, idxs=idx_a,
                                     transforms=M_a, blend_method=blend_method,
                                     interp_method=interp_method,
                                     launch_params=klp_a)
                ...
                klp_b = launch_params_for_volume(out_shp_b, *block_sizes)
                assemble_b = partial(assemble_stitch_and_save_gpuq,
                                     out_shp=out_shp_b, idxs=idx_b,
                                     transforms=M_b, blend_method=blend_method,
                                     interp_method=interp_method,
                                     launch_params=klp_b)
                ...
                for channel in stitch_channels:
                    load_a = partial(_load_function, 
                                     self._root_fldr, 'a', channel)
                    outp_a = os.path.join(
                        out_fldr, _make_2dto1d_fname('a', gidx, channel)
                    )
                    fut_a = executor.submit(assemble_a, 
                                            outp_a, gpu_queue, load_a)
                    load_b = partial(_load_function,
                                     self._root_fldr, 'b', channel)
                    outp_b = os.path.join(
                        out_fldr, _make_2dto1d_fname('b', gidx, channel)
                    )
                    fut_b = executor.submit(assemble_b,
                                            outp_b, gpu_queue, load_b)
                    stitch_futures.append(fut_a)
                    stitch_futures.append(fut_b)
            stitch_iterator = concurrent.futures.as_completed(stitch_futures)
            if verbose:
                stitch_iterator = tqdm(stitch_iterator, 
                                       total=len(stitch_futures),
                                       desc="Assembling Views")
        ...
        with open(os.path.join(out_fldr, 'grid1d_params.json'), 'w') as f:
            json.dump({'overlap_axis' : self.overlap_axes[out_index],
