...
from ..interp.affine import (
    maxblend_into_existing,
...
def assemble_stitch(
    ...
    for idx, mat in zip(idxs, transforms):
        ...
        if blend_method == "max":
            maxblend_into_existing(out, vol, T, interp_method, launch_params)
        elif blend_method == "mean":
            meanblend_into_existing(sum, cnt, vol, T, interp_method, launch_params)
    if blend_method == "max":
        return out
    else:
...
def assemble_stitch_and_save(
    ...
    if out_path[-4:] == "zarr":
        ...
        tifffile.imwrite(
            out_path, stitch, bigtiff=True, imagej=True, metadata={"axes": "ZYX"}
        )
    else:
        raise ValueError("invalid output extension")


def assemble_stitch_and_save_gpuq(
    out_path: os.PathLike,
    queue,
    load_fun: LoadFunction,
    out_shp: List[int],
...
def accumulate_transforms_1d(
    tile_shape: Tuple[int, int, int], overlap_axis: int, len_rest: int, *mats
):
    M = [mats[0]]
    ...
    rel_shift = [numpy.floor(g) if g < 0 else numpy.ceil(g) for g in rel_shift]
    ...
    shp_lrg = numpy.amax(numpy.stack(list(map(out_shp_fun, new_mats)), axis=0), axis=0)
    ...
    sub_val = out_shp[overlap_axis] - tile_shape[overlap_axis]
    for nm in new_mats:
...
class Grid2D:
    def __init__(
        ...
        self._len_overlaps = [
            int(math.ceil(d * po)) for d, po in zip(self._dims, self._pct_overlap)
        ]
    ...
    def one_dimensionalify(
        ...
        **opt_kwargs,
                    ...
                    for grpid, idx0, idx1 in grp:
                        ...
                        if verbose:
                            ...
                            pbar.set_postfix_str(
                                f"G{grpid:d},{idx0:d}->{idx1:d}::met={1.0 - res.fun:.3f}"
                            )
        else:  # parallel
            futures_to_pairs = {}
        ...
        for grpid in range(len(pair_idx_groups)):
            ...
            out_lists[grpid].sort(key=lambda t: [s for s, i in idxoe if i == t[0]][0])
        # now do the accumulation and then stitch together the volumes
        load_all = lambda i: self._load_fun(
            i,
            [
                slice(None),
            ]
        ...
        for grpid, out_list in enumerate(out_lists):
            ...
            new_mats, out_shp = accumulate_transforms_1d(
                ...
                self._overlap_axes[reduction_index],
                self._len_rests,
                *[m for _, m in out_list],
            )
            out_path = os.path.join(out_fldr, "tile{:03d}.zarr")
            if executor is None:
                assemble_stitch_and_save(
                    out_path,
                    load_all,
                    out_shp,
                    idxs,
        ...
        with open(os.path.join(out_fldr, "props.json"), "w") as f:
            json.dump({"overlap_axis": out_overlap, "pct_overlap": out_pct_ov}, f)
        return out_fldr
