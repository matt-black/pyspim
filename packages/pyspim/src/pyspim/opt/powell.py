...
def powell_record(F : Callable[[NDArray],float], 
                  x : NDArray, rec_path : os.PathLike,
                  max_iter : int = 30, search_incr : float = 0.1, 
    ...
    try:
        state, n_iter = powell(F, x, max_iter, search_incr, tol, 
                               rec_queue, verbose)
...
def powell(F : Callable[[NDArray],float], x : NDArray, 
           max_iter : int = 30, search_incr : float = 0.1, tol : float=1e-6,
           rec_queue : Queue|None = None, 
           verbose : bool = False) -> Tuple[NDArray, int]:
    ...
    with (tqdm(total=100) if verbose else nullcontext) as pbar:
        for i in trange(max_iter, desc="cycle", leave=False):
            ...
            for j in trange(n_par, leave=False, desc="parloop"):
                v = u[j]
                f = lambda s : _f1(v, s)
                a, b = goldsect_bracket(f, 0., search_incr)
