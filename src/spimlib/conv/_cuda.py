...
with open(__conv3_module_path, 'r') as f:
    __conv_module_txt = f.read()


__kernel_names = ('convZKernel<unsigned short>', 'convZKernel<float>',
                  'convRKernel<unsigned short>', 'convRKernel<float>',
                  'convCKernel<unsigned short>', 'convCKernel<float>')
...
def _cuda_parameters_for_kernel_radius(kernel_radius : int):
    ...
    return ((z_blockdim_x, z_blockdim_y, z_result_steps, z_border_pix),
...
def make_conv_module(kernel_radius : int,
    ...
    txt = module_txt.format(kernel_radius=kernel_radius,
...
def _launch_pars_z(sze_z : int, sze_r : int, sze_c : int, 
                   z_result_steps : int, 
                   z_blockdim_x : int, 
                   z_blockdim_y : int):
...
def _launch_pars_r(sze_z : int, sze_r : int, sze_c : int,
                   r_result_steps : int,
                   r_blockdim_x : int,
                   r_blockdim_y : int):
...
def _launch_pars_c(sze_z : int, sze_r : int, sze_c : int,
                   c_result_steps : int,
                   c_blockdim_x : int,
                   c_blockdim_z : int):
...
def calc_launch_params(kernel_radius : int, 
    ...
    launch_z = _launch_pars_z(sz, sr, sc,
                              z_result_steps, z_blockdim_x, z_blockdim_y)
    r_blockdim_x, r_blockdim_y, r_result_steps, _ = r_pars
    launch_r = _launch_pars_r(sz, sr, sc,
                              r_result_steps, r_blockdim_x, r_blockdim_y)
    c_blockdim_x, c_blockdim_z, c_result_steps, _ = c_pars
    launch_c = _launch_pars_c(sz, sr, sc,
                              c_result_steps, c_blockdim_x, c_blockdim_z)
    return launch_z, launch_r, launch_c


_TupleCLP = Tuple[CuLaunchParameters,CuLaunchParameters,CuLaunchParameters]
def convolve_3d(x : cupy.ndarray, 
    ...
    convz = cuda_module.get_function('convZKernel<{:s}>'.format(dtype_str))
    convz(launch_z[0], launch_z[1], (out, x, kernel_z, *shp))
    convr = cuda_module.get_function('convRKernel<{:s}>'.format(dtype_str))
    convr(launch_r[0], launch_r[1], (out, x, kernel_y, *shp))
    convc = cuda_module.get_function('convCKernel<{:s}>'.format(dtype_str))
