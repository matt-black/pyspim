...
from qtpy.QtWidgets import (
    ...
    QProgressBar, QSpinBox, QDoubleSpinBox, QLineEdit, QFileDialog
)
...
class DeconvolutionWorker(QThread):
    ...
    def __init__(self, a_registered, b_registered, psf_a, psf_b, 
                 chunk_size, overlap, iterations, regularization, 
                 noise_model='additive', output_path=None):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            ...
            a_zarr = zarr.creation.open_array(
                a_zarr_path, mode='w', shape=self.a_registered.shape,
                dtype=np.uint16, fill_value=0
            )
            a_zarr[:] = self.a_registered
            
            b_zarr = zarr.creation.open_array(
                b_zarr_path, mode='w', shape=self.b_registered.shape,
                dtype=np.uint16, fill_value=0
            )
            ...
            out_zarr = zarr.creation.open_array(
                out_zarr_path, mode='w', shape=self.b_registered.shape,
            ...
            deconvolve_chunkwise(
                a_zarr, b_zarr, out_zarr,
                self.chunk_size, self.overlap,
                psf_a_gpu, psf_b_gpu,
                psf_a_flipped, psf_b_flipped,
                self.noise_model, self.iterations, self.regularization,
                False, None, 0, 0, True
            )
...
class DeconvolutionWidget(QWidget):
    ...
    def _check_ready(self):
        ...
        ready = (self.input_data is not None and 
    ...
    def deconvolve_data(self):
        ...
        self.worker = DeconvolutionWorker(
            a_registered, b_registered, self.psf_a, self.psf_b,
            chunk_size, overlap, iterations, regularization, noise_model
        )
